...
class PermissionService {
  ...
  Future<LocationPermission> requestLocationPermission({
    ...
    try {
      ...
      if (!serviceEnabled) {
        developer.log('PermissionService: Location services not enabled');
        return LocationPermission.denied;
      }

      // Check current permission
      LocationPermission permission = await getCurrentPermission();
      developer.log('PermissionService: Current permission: $permission');

      // Request permission if needed
      if (permission == LocationPermission.denied) {
        permission = await Geolocator.requestPermission();
        developer.log('PermissionService: Requested permission result: $permission');
      }
      ...
      switch (permission) {
        case LocationPermission.always:
          developer.log('PermissionService: Location permission granted (always)');
          break;
        ...
        case LocationPermission.denied:
          developer.log('PermissionService: Location permission denied');
          break;
        case LocationPermission.deniedForever:
          developer.log('PermissionService: Location permission permanently denied');
          break;
        case LocationPermission.unableToDetermine:
          developer.log('PermissionService: Unable to determine location permission');
          break;
      }

      return permission;
    } catch (e) {
      developer.log('PermissionService: Error requesting permission: $e');
      return LocationPermission.denied;
    }
  }
  ...
  Future<bool> openLocationSettings() async {
    try {
      ...
      developer.log('PermissionService: Error opening location settings: $e');
      return false;
    }
  }
